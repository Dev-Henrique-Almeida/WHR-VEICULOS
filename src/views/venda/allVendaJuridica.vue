  ...
  <div>
    <template v-if="tela === 1">
      <v-container>
        ...
        <v-card>
          <v-card-title>
            Selecione a venda que deseja obter informações:
            <v-spacer></v-spacer>
      ...
      <v-container>
        <v-card color="grey darken-4">
          <v-container>
            ...
            <h2> Vendedor: </h2>
            <template>
              <v-data-table :headers="headersFuncionario" :items="venda" hide-default-footer class="elevation-1">
              </v-data-table>
            </template>
            <br />
            <h2> Cliente: </h2>
            <template>
              <v-data-table :headers="headersClienteJuridico" :items="venda" hide-default-footer class="elevation-1">
              </v-data-table>
            </template>
            <br />
            <h2> Veiculo: </h2>
            <template v-if="(condicaoVeiculo === 'Veiculo Usado')">
              <v-data-table :headers="headersVeiculoUsado" :items="venda" hide-default-footer class="elevation-1">
              </v-data-table>
            </template>
            <template v-if="(condicaoVeiculo === 'Veiculo Novo')">
              <v-data-table :headers="headersVeiculoNovo" :items="venda" hide-default-footer class="elevation-1">
              </v-data-table>
            </template>
            <br />
            <h2> Informações do pagamento: </h2>
            <template>
              <v-data-table :headers="headersPagamento" :items="venda" hide-default-footer class="elevation-1">
              </v-data-table>
            </template>
          </v-container>
        </v-card>
...
import CadastroOrdemVendaPessoaJuridicaService from '@/service/CadastroOrdemVendaPessoaJuridicaService';
export default {
  data() {
    return {
      singleSelect: true,
      ...
      headers: [
        ...
        { text: 'Cliente', value: 'cliente.nomeEmpresarial' },
        { text: 'CNPJ Cliente', value: 'cliente.cnpj' },
      ...
      headersClienteJuridico: [
        { text: 'ID', value: 'cliente.id' },
        { text: 'Nome Empresarial', value: 'cliente.nomeEmpresarial' },
        { text: 'CNPJ', value: 'cliente.cnpj' },
        { text: 'Contato', value: 'cliente.contato' },
  ...
  methods: {
    loadAll() {
      this.tela = 1;
      CadastroOrdemVendaPessoaJuridicaService.getAll().then(
        response => {
