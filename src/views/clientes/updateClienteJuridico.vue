<template>
    <div>
        <v-data-table :headers="headers" :items="desserts" sort-by="calories" class="elevation-1" :search="search">
            <template v-slot:top>
                <v-toolbar flat>
                    <v-toolbar-title>Atualizar Cliente Juridico</v-toolbar-title>
                    <v-divider class="mx-4" inset vertical></v-divider>
                    <v-spacer></v-spacer>
                    <v-dialog v-model="dialog" max-width="500px">
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn color="black" dark class="mb-2" v-bind="attrs" @click="cadastrarCliente" v-on="on">
            ...
            <v-text-field v-model="search" append-icon="mdi-magnify" label="Buscar" single-line hide-details>
                                ...
                                <v-container>
                                    <v-row>
                                        <v-col cols="12" sm="6" md="4">
                                            <v-text-field v-model="editedItem.nomeEmpresarial" :rules="nameRules"
                                                label="Nome"></v-text-field>
                                        </v-col>
                                        <v-col cols="12" sm="6" md="4">
                                            <v-text-field v-model="editedItem.contato" :rules="telefoneRules"
                                                v-mask="'(##) #####-####'" type="text" label="Contato"></v-text-field>
                                        </v-col>
                                        <v-col cols="12" sm="6" md="4">
                    ...
                    <v-dialog v-model="dialogDelete" max-width="530px">
                        <v-card>
                            <v-card-title class="text-h9"> Você tem certeza que deseja remover esse cliente?
                            </v-card-title>
                            <v-card-actions>
                                <v-spacer></v-spacer>
...
export default {
    ...
    methods: {
        ...
        deleteItemConfirm() {
            ...
            CadastroClienteJuridicoService.deleteById(this.desserts[this.editedIndex].id).then(
                response => {
