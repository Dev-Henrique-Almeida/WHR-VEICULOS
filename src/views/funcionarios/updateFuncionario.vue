<template>
    <v-data-table :headers="headers" :items="desserts" sort-by="calories" class="elevation-1">
        <template v-slot:top>
            <v-toolbar flat>
                <v-toolbar-title>Atualizar Funcionário</v-toolbar-title>
                        ...
                        <v-card-text>
                            <v-container>
                                <v-row>
                                    ...
                                    <v-col cols="12" sm="6" md="4">
                                        <v-select :items="cargos" v-model="editedItem.cargo" label="Cargos"></v-select>
                                    </v-col>

                                    <v-col cols="12" sm="6" md="4">
                                        <v-text-field v-model="editedItem.salario" v-mask="'#####'"
                                            :rules="salarioRules" type="number" label="Salário">
                                        </v-text-field>
                                    </v-col>
                ...
                <v-dialog v-model="dialogDelete" max-width="530px">
                    <v-card>
                        <v-card-title class="text-h9">Você tem certeza que deseja remover esse Funcionário?
                        </v-card-title>
                        <v-card-actions>
                            <v-spacer></v-spacer>
...
export default {
    ...
    computed: {
        formTitle() {
            return this.editedIndex === -1 ? 'New Item' : 'Editar Funcionário'
        },
    },
    watch: {
        dialog(val) {
